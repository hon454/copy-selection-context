...
별도 **Copy GitHub/GitLab Permalink** 액션은 백그라운드에서 시스템 Git으로 일반 저장소 또는 linked worktree의 원본 HEAD 파일을 검증합니다. **Git은 퍼머링크에만 필요합니다.** Git이 없거나 실행할 수 없으면 이 액션만 지역화된 오류를 표시하며, 일반·컬렉션·이력·상태 표시줄 복사는 계속 사용할 수 있습니다. 플러그인이 Git을 설치하거나 PATH·Git 설정을 변경하지는 않습니다.
