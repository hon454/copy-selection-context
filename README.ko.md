...
- **세션 컨텍스트 컬렉션** — 여러 파일을 추가하고 컬렉션을 열어 고정된 캡처를 검토·정리한 뒤 전체 출력 복사
- **GitHub/GitLab 퍼머링크** — 일반 결과와 동일한 로컬 이력 및 복사 피드백과 함께 선택 라인의 Git 퍼머링크 복사
...
GitHub의 `v1.5.0` 릴리스는 해당 플러그인 버전이 출시되었음을 기록합니다. GitHub 릴리스가 제공된다는 사실만으로 JetBrains Marketplace 승인이나 공개 등록을 의미하지 않으며, 두 상태는 별개입니다.
...
| 컨텍스트 컬렉션에 추가 | 프로젝트 세션에 선택 영역 또는 현재 줄 캡처 |
| 컨텍스트 컬렉션 열기 | 프로젝트 세션 컬렉션 열기·검토·정리 |
| 컨텍스트 컬렉션 전체 복사 | 활성 에디터 없이 컬렉션 복사 |
...
별도 **Copy GitHub/GitLab Permalink** 액션은 백그라운드에서 시스템 Git으로 일반 저장소 또는 linked worktree의 원본 HEAD 파일을 검증합니다. **Git은 퍼머링크에만 필요합니다.** upstream Git 2.45 이상에서 제공하는 기능, 특히 `--no-lazy-fetch`와 `check-attr --source`가 필요하며 실제 실행 파일이 필수 옵션을 지원해야 합니다. Git이 없거나 실행할 수 없거나 필수 옵션을 거부하면 각각 구분된 지역화 오류를 표시합니다. 기능 미지원이라면 Git을 업그레이드하고 IDE가 업그레이드된 실행 파일을 사용하는지 확인하세요. 일반·컬렉션·이력·상태 표시줄 복사는 계속 사용할 수 있습니다. 플러그인이 Git을 설치하거나 PATH·Git 설정을 변경하지는 않습니다.

새 파일이나 이름 변경 후의 경로처럼 HEAD에 없는 경로는 퍼머링크를 복사할 수 없습니다. 현재 편집기 문서가 HEAD와 같으면 추가 확인 없이 복사합니다. 저장하지 않은 편집, 디스크에 저장한 편집, staged 편집 등으로 내용이 다르면 현재 행 번호에 HEAD의 다른 코드가 있을 수 있음을 한 번 확인합니다. **기본 선택은 취소입니다.** **Permalink 복사**를 선택하면 캡처한 HEAD SHA와 현재 caret 행 범위를 사용하며 행 번호를 자동으로 맞추지 않습니다. 검증이나 확인 중에 감지한 변경은 해당 요청을 무효화합니다.

프로젝트 전체에서 가장 최근에 실행한 관리 대상 복사 요청만 결과를 게시할 수 있습니다. Git 조회는 로컬 객체를 읽으며 replace refs와 암묵적 fetch를 비활성화합니다. 지원하지 않거나 누락된 객체, 바이너리 또는 디코딩할 수 없는 내용은 안전하게 실패합니다. 파일 저장·commit·stash·fetch·push를 자동으로 실행하지 않으며, 퍼머링크가 해당 commit의 push 여부나 원격 접근 가능성을 보장하지는 않습니다.

### 이력, 알림 및 상태 표시

- 성공한 일반 경로/코드 복사와 Git 퍼머링크 복사는 프로젝트별 이력 앞에 항목을 추가합니다. `Ctrl+Alt+H`를 누르면 저장된 항목을 볼 수 있고, 항목을 선택하면 전체 내용을 다시 복사합니다.
...
- 성공한 일반 복사 또는 Git 퍼머링크 복사는 활성 에디터의 거터 표시를 교체하고 상태 표시줄 위젯에 prefix를 포함해 최대 40자인 단일 라인, Unicode-safe, markup-escaped 미리보기를 표시합니다. 위젯을 클릭하면 플러그인에서 가장 최근에 성공한 전체 값을 다시 복사합니다. 퍼머링크는 로컬 분석이나 리뷰 프롬프트 집계를 증가시키지 않습니다.
...
### 세션 컨텍스트 컬렉션 (1.5.0부터 제공)

에디터의 Copy Selection Context 하위 메뉴 또는 Find Action에서 **컨텍스트 컬렉션에 추가**를 실행하면 여러 파일의 선택 영역을 수집할 수 있습니다. 각 커서의 선택 영역이나 현재 줄을 저장하지 않은 내용까지 캡처하며, 클립보드와 에디터 포커스는 그대로 유지합니다. 기본 단축키는 없으며 Keymap에서 지정할 수 있습니다.

원문, 경로, 파일명, 언어, 줄 범위, 캡처 번호와 시각은 수집 당시 값으로 고정됩니다. 변경 없는 재수집은 중복으로 건너뛰고 코드가 바뀌면 별도 스냅샷을 추가합니다. 상대·절대 경로 설정만 바꾼 재수집도 중복이며 기존 표시 경로를 유지합니다. 이름 변경·삭제 상태는 캡처 내용과 별도로 추적합니다.
...
### [컬렉션 작업 흐름과 미리보기](https://github.com/hon454/copy-selection-context/issues/74)

추가한 뒤 하위 메뉴나 Find Action에서 **컨텍스트 컬렉션 열기**를 실행하면 에디터 없이도 오른쪽 도구 창을 엽니다. 추가할 때 에디터 포커스를 유지하며 창을 자동으로 열지 않습니다. 열기 액션에는 기본 단축키가 없고 Keymap에서 지정할 수 있습니다. 기존 단일 복사 단축키는 별개입니다.

고정된 캡처 번호·시각·경로·범위로 목록을 확인하세요. 방향키로 선택하고 위/아래 이동으로 선택 항목을 옮깁니다. Delete는 목록에 포커스가 있을 때만 항목을 삭제합니다. 전체 비우기의 기본값은 취소이며 확인 중 컬렉션이 바뀌면 다시 확인합니다. 두 읽기 전용 뷰어에서 포커스·텍스트 선택·일반 복사를 지원합니다.

**코드 포함**은 단일 복사 설정과 독립적입니다. **출력 형식 설정…**에서 기존 형식 설정을 엽니다. 원문과 최종 출력의 UTF-8 바이트를 구분합니다. 계산 중에는 이전 출력을 지우고, 항목 출력이 비거나 4 MiB를 초과하면 전체 복사를 막습니다. 경고 사유는 공통 확인창을 열기 전에도 표시합니다.

원본이 바뀌거나 삭제되어도 캡처 당시 코드는 유지됩니다. 원본 상태는 관찰된 변경을 뜻하며 동일함을 보장하지 않습니다. 복사하거나 창을 숨겨도 유지되며 프로젝트/플러그인 종료 시 폐기됩니다. 컬렉션은 영구 복사 히스토리에 기록되지 않습니다. OS·외부 클립보드 기록은 별개입니다.

캡처를 준비한 뒤 하위 메뉴나 Find Action에서 **컨텍스트 컬렉션 전체 복사**를 실행하세요. 활성 에디터가 없어도 되며 기본 단축키는 없습니다. 현재 형식·템플릿·공백 정리 설정을 캡처된 경로와 코드에 적용합니다. 기본 형식은 같은 위치의 스냅샷에 고정 캡처 번호와 UTC 시각을 붙이며, 사용자 템플릿은 기존 치환을 유지합니다. 항목 출력이 비면 복사를 막습니다. 출력이 256 KiB를 넘으면 확인하고 4 MiB를 넘으면 차단하며, 스냅샷·참조·크기 경고를 한 번에 확인합니다. 복사 후에도 컬렉션을 유지하고 히스토리나 gutter 표식을 추가하지 않습니다. 알림 설정, 선택적 통계, 독립 리뷰 조건은 적용합니다. 히스토리·상태 재복사를 포함해 프로젝트 간 가장 나중 플러그인 복사 요청이 우선하며, 기본 Copy와 외부 클립보드 히스토리는 이 순서 조정 범위 밖입니다. [#75](https://github.com/hon454/copy-selection-context/issues/75)의 [출력 계약](docs/development/context-collection-output-contract.md)과 [예제 선택 영역](docs/samples/context-collection/README.md)을 참고하세요.

![에디터 옆 컨텍스트 컬렉션의 여러 파일 캡처와 바이트 수](docs/images/context-collection-overview.png)

여러 파일의 캡처 목록과 원문·최종 출력 바이트를 에디터 옆에서 확인합니다.

![수정 전후 스냅샷 표식이 있는 캡처 원문과 최종 출력](docs/images/context-collection-preview.png)

수정 전후의 원문을 유지하고 기본 형식의 최종 출력에 캡처 표식을 붙입니다.

![한 항목과 262145 UTF-8 바이트의 실제 크기 확인창](docs/images/context-collection-size-warning.png)

원문 262144바이트 한 항목이 최종 출력 262145 UTF-8 바이트를 만듭니다. 공통 확인창의 기본값은 취소입니다.
