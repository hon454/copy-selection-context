...
| Hide and no-editor state | Hide/reopen at 3b0a7fa retained four items, selection and 315-byte output. At 4de4caf, `final-no-editor-ax.txt` confirms closed editors and the collection stripe, but does not show the open collection, retained items or Copy All. Actual no-editor retention and copying remain pending coordinator verification. Earlier no-editor opening was exercised; the platform fixture separately checks action availability without an editor. |
