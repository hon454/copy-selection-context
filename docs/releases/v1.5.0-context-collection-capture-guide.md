...
| Hide and no-editor state | Hide/reopen at 3b0a7fa retained four items, selection and 315-byte output. At 4de4caf, `final-no-editor-ax.txt` confirms closed editors and the collection stripe, but does not show the open collection, retained items or Copy All. Coordinator subsequently verified actual no-editor retention and Find Action copying on the final main-equivalent ZIP: `build/evidence/integrated-b70f8e2/manual/03-retained-no-editor-ax.txt` shows1item/16raw after tab closure,04 records copying, and05 records normal clipboard paste of17characters/1LF matching the17byte output. Earlier no-editor opening was exercised; the platform fixture separately checks action availability without an editor. |
