...
Gradle separates reusable pure unit execution (`test`) from IntelliJ application and editor-fixture execution (`platformTest`). `CopySelectionActionFixtureTest` and `CopyHistoryPersistenceTest` run through `platformTest` with one class per JVM; `allTests` is the complete local and CI aggregate. CI invokes the aggregate with `--continue` and uploads XML and HTML report directories for both test tasks.
