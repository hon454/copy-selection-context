...
`GitProcessRunner` discovers system `git`/`git.exe` on each request and invokes argv with explicit root/SHA, no replacement refs, no lazy fetch, no optional locks and literal paths. Upstream Git 2.45 supplies the newest required option; the actual safe query detects option-parser rejection as a distinct unsupported-capability failure, without relying on a version string. The child environment removes inherited Git repository/object/config/helper controls, fixes the child locale for bounded option diagnostics, and disables all transports. Repository-owned worktree/common-dir and local alternates remain intact. Both pipes drain with independent bounds, finite timeouts and owned process/stream cleanup. Unsupported local queries fail without fallback, helper/filter execution or network. Platform/coroutine cancellation propagates after cleanup through every pre-publication boundary.
