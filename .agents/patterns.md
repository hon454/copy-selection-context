...
Permalink publication requires `GitHeadTargetValidator` preparation first. Capture the full bounded unsaved document once and keep code out of the prepared URL/range result. HEAD-absent and unknown targets fail; equal document content is clean; dirty content asks once with Cancel as the default. Perform original SHA/blob queries and post-confirmation HEAD revalidation in the background, then validate the same `GitPermalinkLifetime` and application request inside the publisher's final EDT transaction. Never convert `ProcessCanceledException`/`CancellationException` into a Git failure, log, or retry. Only the permalink path may discover system Git; use `GitProcessRunner`'s local-only argv/environment limits rather than shell, diff, textconv, filters or transports. See [the Git contract](../docs/development/git-permalink-contract.md) for the external-HEAD snapshot boundary.
...
Use `Content.setDisposer(panel)` for tool-window resources. Select list entries by stable ID; do not select/focus new captures while the user is editing. Restrict Delete to the list input map. Plain-text viewers must remain read-only, focusable and independent of the Copy All command. Prepare detached documents and ContextCollectionTextLayout geometry off EDT. Keep all Swing components on EDT, install only a current generation through ToolWindowManager.invokeLater, and clear request payload/document references on cancellation or disposal, including requests with an EDT install still queued. The custom View must use prepared dimensions and indexed visible geometry; never reinstate whole-line Swing layout or obtain Copy All payloads from the view.
